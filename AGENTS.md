...
- **Current phase:** Milestone 2-A next (rulestrings).
- **Active branch:** `main`
...
- Default preset `conway` (`B3/S23`); alternate `ant_colony` (`B3/S234`) in M2-A.
...
- `src/rules.lua`: named rulestring presets and `Bx/Sy` parser (**M2-A**).
...
- **Today:** `src/grid.lua` — Conway rules, toroidal wrap, `computeNext` immutability, `step`.
- **M2-A adds:** `tests/rules_spec.lua` — parser, presets, Ant Colony behavior.
