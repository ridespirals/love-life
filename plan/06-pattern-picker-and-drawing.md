...
**Status:** not started; comes after Phase 3.

## Development order

1. **4a — Pattern picker**: catalog list + Load (no drawing yet).
2. **4b — Board drawing**: click-to-draw + draft + Save (needs Phase 0 idle animation + input routing already shipped).
...
- Only when `playback` paused (see [`03-playback.md`](03-playback.md)) and no pane consuming clicks
...
| `src/input/board.lua` | planned | Phase 4 (superseded by Phase 7 Draw tool — see notes above) |
