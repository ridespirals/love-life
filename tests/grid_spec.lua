...
test("resize centers prior cells in a larger grid", function()
  local world = grid.create(3, 3)
  grid.setAlive(world, 2, 2, true)
  local resized = grid.resize(world, 5, 5)
  assert.equal(resized.rows, 5)
  assert.equal(resized.cols, 5)
  assert.isTrue(grid.isAlive(resized, 3, 3))
  assert.equal(aliveCount(resized), 1)
end)

test("resize crops centered cells when shrinking", function()
  local world = grid.create(5, 5)
  grid.setAlive(world, 3, 3, true)
  local resized = grid.resize(world, 3, 3)
  assert.isTrue(grid.isAlive(resized, 2, 2))
end)
